...
MARKER="$HOME/.config/edulab/desktop-style-v25.done"
...
'
  ...
  if [[ -f "$SCRIPT_DIR/edulab-quick-settings-menu.py" ]]; then
    run install -m 0755 "$SCRIPT_DIR/edulab-quick-settings-menu.py" /usr/local/bin/edulab-quick-settings-menu
  else
    log "CẢNH BÁO: Không tìm thấy scripts/edulab-quick-settings-menu.py, bỏ qua quick settings tùy biến."
  fi
